...
2 23087 071.0530 150.4278 0008158 020.1711 081.1537 14.13699811110975`;
    ...
    let map = new EsriMap({
      ...
      layers: [citiesLayer, analysisGraphicsLayer3D],
    });
    ...
    var messageLayer = new GraphicsLayer();
    var satelliteTracks = new GraphicsLayer();

    map.addMany([satelliteLayer, satelliteTracks, messageLayer]);
    ...
    async () => {
      for (let i = 0; i < (await messages.count); i++) {
        // make all of the message airplanes
        // messages[i]

        var graphic = new Graphic({
          geometry: new Point(
            messages[i].from.longitude,
            messages[i].from.latitude,
            1000000
          ),
          symbol: {
          ...
          attributes: {
            message: messages[i],
            heading: {
              x: Math.random(), // point to your destination
              y: Math.random(),
            },
          },
          popupTemplate: template,
        });
        messageLayer.add(graphic);
      }
    };
    ...
    view.on("click", function (evt) {
      ...
      locatorTask.locationToAddress(params).then(
        async function (response) {
          ...
          await showAddress(response, evt.mapPoint);
        },
        async function (err) {
          // Show no address found
          await showAddress("No address found.", evt.mapPoint);
        }
      );
    });
    requestAnimationFrame(moveSatellites);
    ...
    async function showAddress(response, pt) {
  ...
  function sendMessage() {
    ...
    const hash = geofire.geohashForLocation([view.center.latitude, view.center.longitude]);
